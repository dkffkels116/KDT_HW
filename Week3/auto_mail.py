...
crawler = NaverNewsCrawler(input('Input: '))

#### 수집한 데이터를 저장할 엑셀 파일명을 input을 이용해 입력받아 ? 부분에 넣으세요
crawler.get_news(input('FileName: '))
...
SMTP_PORT = 465
SMTP_USER = 'dkffkels116@gmail.com'
SMTP_PASSWORD = 'rlaalstjr1!'
...
ws = wb.active
ws.append(['이름', '이메일'])
ws.append(['김민석', 'dkffkels116@gmail.com'])
wb.save('email_list.xlsx')

#### 엑셀 파일의 정보를 읽어올 수 있는 모듈을 import하세요.
from openpyxl import load_workbook

rb = load_workbook('email_list.xlsx')
data = rb.active
# print(data)
# print('#########################')
#### email_list.xlsx 파일을 읽어와 해당 사람들에게 수집한 뉴스 정보 엑셀 파일을 send_mail 함수를 이용해 전송하세요.
for row in data.iter_rows(min_row = 2):
    print(row[0].value + "에게 " + row[1].value + "로 전송")
# area = data[str('B2')].value
# print(area)
    send_mail(row[0].value, row[1].value, '업무 test' , '제발 되라', 'save_data.xlsx')